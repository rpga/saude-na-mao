...
public class BDCore extends SQLiteOpenHelper {	
	private static final String nome_BD = "Saude";
	...
	public void onCreate(SQLiteDatabase bd){
		...
		bd.execSQL("create table uds(_id integer primary key autoincrement, " +
				...
				"cidade text not null, numero integer, bairro text not null," +
				"foreign key(cod_tipo) references tipo(_id));");	
		bd.execSQL("create table telefone(_id integer, telefone integer, primary key(_id,telefone), foreign key(_id) references uds(_id));");
		bd.execSQL("create table possui(_id integer, cod_uds integer, primary key(_id,cod_uds), foreign key(_id) references uds(_id)," +
				" foreign key(cod_uds)references especialidade(_id));");
	}
	@Override
